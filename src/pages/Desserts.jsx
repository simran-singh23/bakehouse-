...
import { dessertItems } from '../data/bakeryData'
...
function Desserts() {
  ...
  const totalPages = Math.ceil(dessertItems.length / itemLimit)
  const start = (page - 1) * itemLimit
  const pageItems = dessertItems.slice(start, start + itemLimit)
  ...
  return (
    <section className="section-wrap">
      <div className="section-title">
        <p>Desserts</p>
        <h2>Pastries, Brownies & Cupcakes</h2>
      </div>

      <div className="mb-8 rounded-xl border border-[#d4c3be]

       bg-white p-6 shadow-sm md:flex md:items-center md:justify-between md:gap-8">
        <div>
          <p className="text-xs font-black uppercase tracking-widest text-[#805062]">Dessert counter</p>

          <h3 className="mt-2 text-2xl font-black text-[#442a22]">Sweet boxes for every small celebration.</h3>
        </div>
        <p className="mt-4 max-w-2xl leading-7 text-[#504441] md:mt-0">

          Choose from brownies, pastries, cupcakes and dessert cups, then open the order page to
        
          send your details directly on WhatsApp.
...
<article key={item.id} className="group overflow-hidden rounded-xl border border-[#d4c3be]/60 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
            ...
            <div className="p-5">
              ...
              <div className="mt-3 flex flex-wrap gap-2 text-sm font-bold">
                ...
                {item.eggless && <span className="rounded-full bg-[#cceacd] px-3 py-1 text-[#334d37]">Eggless</span>}
              </div>
              <p className="mt-3 leading-7 text-[#504441]">{item.desc}</p>
              <NavLink to={`/desserts/${item.id}`} className="btn-main mt-5">
                Order
              </NavLink>
            </div>
          </article>
